...
class NoiseNode(nef.SimpleNode):
    ...
    def __init__(self, frequency, dimension=1):
        ...
        # OMG: this is all we need to change
...
class VectorNoiseNode(nef.SimpleNode):
    """Node to output gaussian noise with mean 0 and standard deviation driven
    by an input signal.
    
    :input scale: scale vector on the output values (modifying standard deviation from base of 1)
    :output noise: vector of noisy values
    """
    ...
    def termination_scale(self, x):
        self.scale = x # gets set to 0.03 by bg_network
