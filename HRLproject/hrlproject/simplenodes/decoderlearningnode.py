...
class DecoderLearningNode(nef.SimpleNode):
    """Implements a decoder learning rule (kind of a hack to get this into Nengo 1.4).
    
    :input error: error signal driving learning
    """
    
    def __init__(self, ens, origin, rate, errorD=1, learning=True, name="DecoderLearningNode"):
    ...
    def tick(self):
        if self.learning:
            ...
            self.origin.setDecoders([[delta[i]+val for i,val in enumerate(old_d)] for delta,old_d in zip(deltas,decoders)])
