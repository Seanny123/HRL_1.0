...
class ContextEnvironment(PlaceCellEnvironment):
    """Environment based on PlaceCellEnvironment, but supplemented with a context signal.
...
#            self.contexts[r] = list(RandomHypersphereVG().genVectors(1, contextD)[0])
    ...
    def update_reward(self):
        # agent is rewarded if it is in the target region associated with the current context
        self.reward = self.defaultreward
        for r in self.rewards:
            if self.context == self.contexts[r] and self.is_in(self.state, r):
                self.reward += self.rewards[r]

        if self.is_in(self.dest, "wall"): # penalize for trying to move into walls
            self.reward = -0.1
...
#        c_encoders = RandomHypersphereVG(True,1.0,1.0).genVectors(N, d) #0.7
    ...
    def calc_optimal_move(self):
        ...
        goal = [c for c in self.contexts if self.contexts[c] == self.context][0]
    ...
    def colour_translation(self, c):
        """Translate box labels into colours (used for interactivemode display)."""
